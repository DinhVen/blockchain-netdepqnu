const StatsCards = ({ stats }) => {
  ...
  return (
    <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
      {cards.map((card, i) => (
        <div key={i} className="bg-white dark:bg-gray-800 rounded-xl border border-[#E2E8F0] dark:border-gray-700 p-4">
          <div className="flex items-center gap-2 mb-2">
            <div className="p-1.5 bg-[#2563EB]/10 text-[#2563EB] rounded-lg">
              {icons[card.icon]}
            </div>
          </div>
          <p className="text-2xl font-bold text-[#0F172A] dark:text-white">{card.value}</p>
